...
AS
    ...
    -- Cursor for available time slots (non–breaks only)
    CURSOR c_slots IS
        SELECT  ts.slot_id,
                ts.day_of_week,
                ts.start_time,
...
BEGIN
    ...
    VALUES (
        'Starting timetable generation for Year ' || p_year || ', Sem ' || p_sem,
        'INFO',
        v_process_id
    );
        ...
        FOR rec_course IN c_courses(rec_class.class_id) LOOP
            ...
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    INSERT INTO TimeTable_Log (msg, log_type, process_id)
                    VALUES (
                        'No faculty found for course ' || rec_course.course_code,
                        'WARNING',
                        v_process_id
                    );
                ...
                BEGIN
                    ...
                    FROM   (
                        ...
                        WHERE  cr.room_type = CASE
                                                  WHEN rec_course.course_type = 'LAB'
                                                  THEN 'LAB'
                                                  ELSE 'LECTURE'
                                              END
                        AND    cr.capacity >= rec_class.NOS
                        ORDER  BY DBMS_RANDOM.VALUE
                    )
                    WHERE  ROWNUM = 1;
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        INSERT INTO TimeTable_Log (msg, log_type, process_id)
                        VALUES (
                            'No suitable room for course ' || rec_course.course_code,
                            'WARNING',
                            v_process_id
                        );
                        v_error_count := v_error_count + 1;
                ...
                BEGIN
                    ...
                    FOR rec_slot IN c_slots LOOP
                        ...
                        IF v_can_use_slot = 1 THEN
                            ...
                            FROM   Timetable t
                            WHERE  t.class_id = rec_class.class_id
                            AND    t.slot_id  = rec_slot.slot_id;
                    ...
                    IF v_slots_allocated < v_slots_needed THEN
                        INSERT INTO TimeTable_Log (msg, log_type, process_id)
                        VALUES (
                            'Partial allocation for ' || rec_course.course_name || ': ' ||
                            v_slots_allocated || '/' || v_slots_needed || ' slots',
                            'WARNING',
                            v_process_id
                        );
                    END IF;
    ...
    INSERT INTO TimeTable_Log (msg, log_type, process_id)
    VALUES (
        'Timetable generation completed. Classes: ' || v_total_classes ||
