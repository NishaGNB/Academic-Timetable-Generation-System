        ...
        FOR rec_slot IN (
            ...
            FOR rec_class IN c_classes LOOP
                DECLARE
                    ...
                    v_max_tries NUMBER := 10; -- Try up to 10 different faculty/room combinations
                    v_allocated BOOLEAN := FALSE;
                BEGIN
                    -- Keep trying until we allocate or run out of options
                    WHILE v_try_count < v_max_tries AND NOT v_allocated LOOP
                        ...
                        EXCEPTION
                            WHEN NO_DATA_FOUND THEN
                                -- No more courses available for this class today, exit retry loop
                                EXIT;
                        END;
                        
                        -- Find faculty for this course (random to try different combinations)
                        BEGIN
                            ...
                            FROM (
                                ...
                                AND cf.fac_id NOT IN (
                                    -- Exclude faculty already teaching at this slot
                        ...
                        -- Find room for this course (random to try different combinations)
                        BEGIN
                            ...
                            FROM (
                                ...
                                AND room_no NOT IN (
                                    -- Exclude rooms already occupied at this slot
